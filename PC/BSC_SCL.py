...
def BPSK_decoder(d, N, frozen_set, p_cross):
    ...
    """SCL initialization"""
    L = 4   # TODO: list size of L =8. From "The Development and Operations of...
    ...
    while not done:
        if depth == log(N, 2):
            ...
            decision_metric = HF.sign_rev(node.beliefs)
            is_frozen = tree.index(node)-(2**log(N, 2)-1) in frozen_set # alternatively var name,
            list_decoders = HF.update_decoders(is_frozen, decision_metric, list_decoders, L)
            node.beliefs = vector(F, [decision_metric])
