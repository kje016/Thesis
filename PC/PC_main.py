...
if __name__ == "__main__":
    cntr, runs = 0, 30
    A = int(sys.argv[1])
    I_IL, channel, p_cross = int(sys.argv[2]), sys.argv[4].upper(), 0.01
    ...
    for p in range(runs):
        ...
        for a_elem in [a]:
            ...
            # print(f"ms-bits = {matching_scheme, [d[a] for a in MS]}")
            #print(f" D:= {d}")
            ...
            if channel == 'BEC':
                ...
                uu = BSC_SCL.decoder(d=yy, N=N, frozen_set=QNF, p_cross=sigma[sigi], MS=MS)
                # print([a.inf_bits for a in uu])
            for dec in uu:
